...
namespace IFilterTextReader
{
    ...
    public class FilterReader : TextReader
    {
        ...
        /// <exception cref="IFOldFilterFormat">Raised when an old <see cref="NativeMethods.IFilter"/> format is used and no filename is supplied</exception>
        /// (e.g. files with a wrong extension)</exception>
        public override int Read(char[] buffer, int index, int count) // Incorrect method to read text, Find by George. Use "ReadToEndWithProperties" instead.
        {
            if (buffer == null)
        ...
        /// <returns></returns>
        public string ReadToEndWithProperties(Dictionary<string, string> dicProperties)
        {
            if (_filter == null)
            {
                return ""; // Formart is not correct, return empty.
            }
            ...
            while (!_done)
            {
                ...
                switch (_chunk.flags)
                {
                    ...
                    case NativeMethods.CHUNKSTATE.CHUNK_VALUE:
                        ...
                        switch (valueResult)
                        {
                            ...
                            case NativeMethods.IFilterReturnCode.FILTER_S_LAST_VALUES:
                                ...
                                if (!string.IsNullOrEmpty(temp))
                                {
                                    string[] pairs = temp.Split(new char[]{ ':' }, 2, StringSplitOptions.RemoveEmptyEntries);
                                    
                                    if(!string.IsNullOrEmpty(pairs[0]))
                                    {
                                        dicProperties[pairs[0].Trim()] = pairs[1].Trim();
                                    }
                                }
                    ...
                    case NativeMethods.CHUNKSTATE.CHUNK_TEXT:
                        ...
                        strBuilder.Append(new string(textBuffer, 0, (int)textLength));
                        break;
                }
                if (Timeout())
                    break;
